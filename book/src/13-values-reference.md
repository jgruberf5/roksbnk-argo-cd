...
| Value | Default | Meaning |
|---|---|---|
| `runner.image` / `runner.tag` | `ghcr.io/jgruberf5/roksbnkctl-tools-runner` / `v1.58.0` | The roksbnkctl runner image. Mirror it for air-gapped hubs. |
| `runner.minVersion` / `runner.allowOlder` | `1.58.0` / `false` | The required roksbnkctl version. Semver tags below it fail at render time; the init hook checks the binary at runtime. |
