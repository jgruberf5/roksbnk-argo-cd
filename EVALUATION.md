...
| `bnk down` destroy ordering: the CNEInstance delete returns without waiting for finalizers and `helm_release.flo` is removed 3 s later, so `f5-bnk` sticks in Terminating until the kubernetes provider's 5-min timeout; roksbnkctl's `freeStuckBNKNamespace` repairs it and retries (deterministic on 2.4, 3/3 teardowns; [roksbnkctl#217](https://github.com/jgruberf5/roksbnkctl/issues/217) — **fixed in v1.55.1**, now the chart's minimum). Fix was: wait for CNEInstance/child-CR deletion (`tfx wait --for delete`) before destroying the operator, or delete the F5 CRs pre-destroy while FLO runs | M | removes ~5 min and an alarming `context deadline exceeded` from every teardown |
